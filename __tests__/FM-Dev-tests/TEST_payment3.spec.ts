...
describe("Test 3 of the Merchant Payment API to payment Authorized", () => {

    let accessToken: string = '';
    let paymentAuthorizationUri = '';
    let paymentToken = '';
    let getStatusLink = '';
    let paymentEndPoint = 'https://test.api.banking-gateway.jigpay.co.uk/api/v1.0/payments/';
    let status = '';

    test("Check the api.jigpay version", async done => {

        request
            .get('https://test.api.jigpay.co.uk/api/version')
    ...
    test("get token", async done => {
        ...
        request
            .post('https://test.api.banking-gateway.jigpay.co.uk/connect/token',
                {
                    form: {
    ...
    test("create payment and save paymentAuthorizationUri", async done => {
        ...
        request
            .post('https://test.api.banking-gateway.jigpay.co.uk/api/v1.0/payments/',
                {
                    body: JSON.stringify({
                        "amount": "3700.00",
